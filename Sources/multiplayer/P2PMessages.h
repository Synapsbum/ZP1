...
#include "loadout.h" // we definitely need this to send the item array
...
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#define P2PNET_VERSION		(0x000004ad + GBWEAPINFO_VERSION + GBGAMEINFO_VERSION + GAMEPLAYPARAM_VERSION)
...
enum pkttype_e
{
  ...
  PKT_C2C_PlayerReloaded,
  PKT_C2S_PlayerUnloadClip,
  PKT_C2S_OpenCase,			//AlexRedd:: case system
  ...
  PKT_C2S_MoveAllItemsToGIReq,	// move all items to GI
...
#if ANTICHEAT && MEMORY_HB
  PKT_C2S_MemoryLog, // aLca :: memory protection hearthbeat :]
#endif
  ...
  PKT_C2S_LearnGearSkinRecipe,//new add skin
  PKT_C2S_GearSkinItem,//new add skin
  ...
  PKT_C2S_AirDropUnlock,

  PKT_S2C_RareBoxUpdate,

  PKT_S2C_CarMapUpdate,
  ...
  PKT_S2C_BarricadeHealth,      //AlexRedd:: barricade health
  ...
  PKT_S2C_BattleRoyaleInfo,//AlexRedd:: BR mode
  ...
  PKT_C2C_ChatMessageBox, // AlexRedd:: for colored Open Case chat message
  PKT_C2C_CaseMessageHUD,   // AlexRedd:: for case HUD message
  ...
  PKT_S2C_UpdateWpnSkinSelected,//new add skin
  PKT_C2S_LearnSkinItem,//new add skin
  ...
  PKT_C2S_ChangeLoadoutReq,	// loadout system
  PKT_C2S_ChangeLoadoutAns,	// loadout system ANS
...
//AlexRedd:: BR mode
struct PKT_S2C_SetGasRadius_s : public DefaultPacketMixin<PKT_S2C_SetGasRadius>
{
	float		gasRadius;
	int			gasStatus;
	bool		isGasSuspended;
...
struct PKT_C2S_SetRespawnData_s : public DefaultPacketMixin<PKT_C2S_SetRespawnData>//AlexRedd:: BR mode used
{
	gp2pnetid_t	PlayerID;
	int spawnSelected;
};

struct PKT_S2C_RespawnPlayer_s : public DefaultPacketMixin<PKT_S2C_RespawnPlayer>//AlexRedd:: BR mode used
{	
	r3dPoint3D	pos;
	float		dir;
...
struct PKT_C2C_PlayerReloaded_s : public DefaultPacketMixin<PKT_C2C_PlayerReloaded>
{
	BYTE		WeaponSlot;
	BYTE		AmmoSlot;
	BYTE		Amount;	// actual number of bullets reloaded
};
...
#if ANTICHEAT && MEMORY_HB
struct PKT_C2S_MemoryLog_s : public DefaultPacketMixin<PKT_C2S_MemoryLog>
{
	int lCode;
	int Status;
	char pName[512];
...
struct PKT_C2S_ChangeLoadoutReq_s : public DefaultPacketMixin<PKT_C2S_ChangeLoadoutReq>
{
	loadout loadout_;
};

struct PKT_C2S_ChangeLoadoutAns_s : public DefaultPacketMixin<PKT_C2S_ChangeLoadoutAns>
{
	// just used to update backpack for now
	int	BackpackSize;
	uint32_t BackpackID;
};

struct PKT_C2S_MoveAllItemsToGIReq_s : public DefaultPacketMixin<PKT_C2S_MoveAllItemsToGIReq>
{
};
...
struct PKT_S2C_InventoryOpAns_s : public DefaultPacketMixin<PKT_S2C_InventoryOpAns>
{
	enum EAns
	{
	  ...
	  ANS_BadTarget,
	  ANS_Fail,
	  ANS_Forbidden,//AlexRedd:: no drop backpack
	  ANS_GIBanned,
	};
	BYTE		OpAns;
...
struct PKT_S2C_CreateNetObject_s : public DefaultPacketMixin<PKT_S2C_CreateNetObject>
{
	...
	float		var6;	//AlexRedd:: barricade health
};
...
struct PKT_C2C_TradeRequest_s : public DefaultPacketMixin<PKT_C2C_TradeRequest>
{
	enum EOp
	{
	  ...
	  TRADE_Forbidden,  // Forbidden items for trade //AlexRedd:: Vip items (nodrop)
	};
...
//new add skin
struct PKT_C2S_LearnGearSkinRecipe_s : public DefaultPacketMixin<PKT_C2S_LearnGearSkinRecipe>
{
	BYTE slotFrom;
};
//new add skin
...
//new add skin
struct PKT_C2S_GearSkinItem_s : public DefaultPacketMixin<PKT_C2S_GearSkinItem>
{
	uint32_t SkinID;
	uint32_t recipeID;
};
...
struct PKT_C2S_AirDropUnlock_s : public DefaultPacketMixin<PKT_C2S_AirDropUnlock>
{
	gp2pnetid_t	lockboxID;	
	int			UnlockAirdrop;
};
...
struct PKT_S2C_CarMapUpdate_s : public DefaultPacketMixin<PKT_S2C_CarMapUpdate>
{
	gp2pnetid_t	carID;
	r3dPoint3D	location;
	float		m_time;
...
struct PKT_S2C_RareBoxUpdate_s : public DefaultPacketMixin<PKT_S2C_RareBoxUpdate>
{
	gp2pnetid_t	boxID;
	uint32_t	itemID;
	r3dPoint3D	location;
...
struct PKT_S2C_BarricadeHealth_s : public DefaultPacketMixin<PKT_S2C_BarricadeHealth>//AlexRedd:: barricade health
{	
	float		barricadeHealth;
};
...
struct PKT_S2C_BattleRoyaleInfo_s: public DefaultPacketMixin<PKT_S2C_BattleRoyaleInfo>
{
	//AlexRedd:: BR mode
	int			alivePlayersLeft;	
};
...
struct PKT_C2C_ChatMessage_s : public DefaultPacketMixin<PKT_C2C_ChatMessage>
{
	...
	char		gamertag[32*2];
	char		msg[128*2]; // actual text
};

struct PKT_C2C_ChatMessageBox_s : public DefaultPacketMixin<PKT_C2C_ChatMessageBox>
{
	char		gamertag[32 * 2];//username	
	DWORD		itemId; //win weapon	
};

struct PKT_C2C_CaseMessageHUD_s : public DefaultPacketMixin<PKT_C2C_CaseMessageHUD>
{
	DWORD		caseItemId;//used case id
	DWORD		winItemId; //win id
	int		quantity;
};
...
struct PKT_S2C_VehicleSpawn_s : public DefaultPacketMixin<PKT_S2C_VehicleSpawn>
{
	...
	DWORD		vehicleOwnerId;// Owner CustomerID
	char		car_owner_user_name[128];//owner user name
	int			isLockedByOwner;// lock/unlock
	bool		isHasItems;//lockbox items
};
