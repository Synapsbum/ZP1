...
class MasterServerLogic : public r3dNetCallback
{
  protected:
	...
	void SendSessionInfo(DWORD CustomerID, DWORD SessionID);
  ...
  public:
	...
	/* FUNCTIONS */
	void RequestGameList(const NetPacketsMaster::GBPKT_C2M_RefreshList_s& n);
	int WaitForGameList();

	void SendJoinGame(DWORD gameServerId, const char* pwd = "");
	void SendJoinQuickGame(const NetPacketsMaster::GBPKT_C2M_QuickGameReq_s& n);
	int WaitForGameJoin();
