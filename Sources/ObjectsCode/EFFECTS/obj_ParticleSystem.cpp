...
void obj_ParticleSystem::UpdateSize()//AlexRedd:: BR mode
{
	if(bSize!=0)
		Torch->ChangeSize( bSize );	
	return;
}
...
obj_ParticleSystem::AppendRenderables( RenderArray ( & render_arrays  )[ rsCount ], const r3dCamera& Cam ) /*OVERRIDE*/
{
	...
	if(CurHUDID == TypesofHUD::T_CameraHUD && !r_hide_icons->GetInt() && ( Cam - GetPosition() ).LengthSq() < idd * idd && g_bEditMode )
	{
...
#ifndef FINAL_BUILD
	if(g_bEditMode && CurHUDID == TypesofHUD::T_ParticleHUD) // in particle editor mode skip collision check with particles
		ObjFlags |= OBJFLAG_SkipCastRay;
#endif
