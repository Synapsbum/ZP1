...
#include "ObjectsCode/weapons/BackPack.h"//new add skin
...
void CUberData::LoadAnimations()
{
	...
	//GetRektBambi [TUTORIAL] Inspecting Gun In FirstPerson
	...
	aid_.InspectWeapon[6] = AddAnimation("FPS_Melee_Show"); //MELEE
	aid_.InspectWeapon[7] = AddAnimation("FPS_Melee_Show"); //SUPPORT
...
CUberEquip::~CUberEquip()
{
	for(int i=0; i<SLOT_Max; ++i)
	{
		...
		SAFE_DELETE(slots_[i].bpk);//new add skin
		slots_[i].wpn = NULL;
		slots_[i].mesh = NULL;
	}
	player = NULL;
...
void CUberEquip::SetSlot(ESlot slotId, BackPack* bpk)
{
	slot_s& sl = slots_[slotId];
	r3d_assert(sl.wpn == NULL); // must be a gear slot
	//r3d_assert(sl.mesh == NULL);

	SAFE_DELETE(sl.bpk); // release old one
	sl.bpk = bpk;
}
...
r3dMesh* CUberEquip::getSlotMesh(ESlot slotId)
{
	if(slots_[slotId].gear)
		return slots_[slotId].gear->getModel(g_camera_mode->GetInt()==2 && player && player->NetworkLocal);
	else if(slots_[slotId].wpn)
		return slots_[slotId].wpn->getModel(true, g_camera_mode->GetInt()==2 && player->NetworkLocal);
	else if (slots_[slotId].bpk)
		return slots_[slotId].bpk->getModel(g_camera_mode->GetInt() == 2 && player->NetworkLocal);
...
int	CUberEquip::IsLoaded()
{
	...
	for( int i = 0 ; i < SLOT_Max; i ++ )
	{
		...
		else if (slots_[i].bpk)
		{
			if (slots_[i].bpk->getModel(isFirstPerson) && slots_[i].bpk->getModel(isFirstPerson)->IsDrawable() == false)
				return false;
		}
...
void CUberEquip::DrawSlot(ESlot slotId, const D3DXMATRIX& world, DrawType dt, bool skin, bool draw_firstperson, const r3dSkeleton* wpnSkeleton, bool ForceTPSWeapon)
{
	if (draw_firstperson)
	{
	...
	r3dMesh* mesh = NULL;
	if (slots_[slotId].gear)
		mesh = slots_[slotId].gear->getMeshSkin(isFirstPerson&&draw_firstperson); //getModel(isFirstPerson&&draw_firstperson);
	else if (slots_[slotId].bpk)
		mesh = slots_[slotId].bpk->getMeshSkin(isFirstPerson&&draw_firstperson); //getModel(isFirstPerson&&draw_firstperson);
	else if (slots_[slotId].wpn)
	{
		if (slots_[slotId].wpn->getCategory() == storecat_UsableItem)
			ForceTPSWeapon = !(g_camera_mode->GetInt() == 2 && player->NetworkLocal);
		mesh = slots_[slotId].wpn->getMeshSkin(isFirstPerson&&draw_firstperson, ForceTPSWeapon); //slots_[slotId].wpn->getModel(true, isFirstPerson&&draw_firstperson);
	}
	else if (slots_[slotId].mesh)
...
void CUberEquip::Draw(const r3dSkeleton* skel, const D3DXMATRIX& CharMat, bool draw_weapon, DrawType dt, bool first_person, bool is_in_vehicle)
{
	if (dt == DT_AURA)
	{
	...
	{
		if ((d_disable_backpacks_draw->GetBool()/* || player->CurLoadout.BackpackID >= 20260 && player->CurLoadout.BackpackID <= 20264*/) && i == SLOT_Backpack)
			continue;

		DrawSlot((ESlot)i, CharMat, dt, true, first_person, NULL);
	}

	if (dt != DT_AURA && !is_in_vehicle)
	{
		...
		if (!first_person)
		{
			if (slots_[SLOT_WeaponBackRight].wpn)
			{
				...
				if (wpn)
				{
					r3dMesh* msh = wpn->getModel(true, true);
			...
			{
				if (slots_[SLOT_WeaponBackLeft].wpn->getCategory() == storecat_MELEE &&
					slots_[SLOT_WeaponBackLeft].wpn->getItemID() != 101306 && // Flashlight
					slots_[SLOT_WeaponBackLeft].wpn->getItemID() != 101347) { // Chainsaw
					...
					bool skinned = false;
					r3dSkeleton* wpnSkel = game_new r3dSkeleton();
					Weapon* wpn = slots_[SLOT_WeaponBackLeft].wpn;
					...
					skel->GetBoneWorldTM("Weapon_BackLeft", &world, CharMat);
					DrawSlot(SLOT_WeaponBackLeft, world, dt, skinned, first_person, wpnSkel);
				}
			}
...
CUberAnim::CUberAnim(obj_Player* in_player, CUberData* in_data)
{
	...
	jumpAnimSpeed           = 1.5f;//AlexRedd:: from 1.9f;
	jumpStartTime           = 0.3f;
...
void CUberAnim::SyncAnimation(int PlayerState, int MoveDir, bool force, const Weapon* weap, bool isInAttmMenu, bool isWantingToInspect)//GetRektBambi [TUTORIAL] Inspecting Gun In FirstPerson
{
	if(weap)
	{
...
// 		// if throwing grenades from idle, switch to idleaim
	...
	if(!isWantingToInspect && isInAttmMenu && CurrentWeapon && IsFPSMode()//GetRektBambi [TUTORIAL] Inspecting Gun In FirstPerson
...
//void CUberAnim::StartSwitchAnim()
//{
//	// check if we already have reloading animation
//	r3dAnimation::r3dAnimInfo* animInfo = anim.GetTrack(switchAnimTrackID);
//	if (animInfo && (animInfo->GetStatus() & ANIMSTATUS_Playing)) {
//		return;
//	}
//
...
//	}
//
//	switchAnimTrackID = anim.StartAnimation(data_->aid_.Switch[1], ANIMFLAG_PauseOnEnd, 0.0f, 1.0f, 0.1f);
//	anim.GetTrack(switchAnimTrackID)->fSpeed = SwitchAnimSpeed;
//
