...
//new add skin
void GearConfig::SetMesh( char* Filename ) const
{
	m_Model = r3dGOBAddMesh(Filename, true, false, true, true );
	//aquireMesh();
}
//new add skin
...
//new add skin
void BackpackConfig::aquireMesh() const
{
	if( !m_Model )
	{
		m_Model = r3dGOBAddMesh(m_ModelPath, true, false, true, true );
...
void BackpackConfig::releaseMesh() const
{
	if( r_allow_ingame_unloading->GetInt() )
	{
		if( m_ModelRefCount == 1 )
