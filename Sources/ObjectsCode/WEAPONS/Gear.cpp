...
void Gear::ChangeGearSKIN(int SkinID)
{
	if (this == NULL)
		return;
...
r3dMesh* Gear::getMeshSkin(bool isFirstPerson)
{
	if (this == NULL)
		return NULL;
	...
	if (TotalSkins>0)
	{
		const BaseItemConfig* SkinCfg = g_pWeaponArmory->getConfig(GearSkins[ActualSkin].itemID);
		if (SkinCfg)
		{
			if (isFirstPerson)
			{
...
void Gear::SetSkins(int SkinsID, int itemID, char* FileName, char* StoreIcon)
{
	GearSkins[SkinsID].itemID = itemID;
	GearSkins[SkinsID].FileName = FileName;
	GearSkins[SkinsID].StoreIcon = StoreIcon;
}

char* Gear::GetIconSkin(int SkinID)
{
	if (SkinID<TotalSkins)
		return GearSkins[SkinID].StoreIcon;
	else
		return "Not found";
}
...
bool Gear::GetSkins()
{
		...
		pugi::xml_node xmlSkinSystem = xmlFile.child("SkinsDB");
	{
		...
		pugi::xml_node xmlSkinGear = xmlSkinsArmory.child("Gear");

		while(!xmlSkinGear.empty())
		{
			...
			if (itemID!=0)
			{
				...
				uint32_t itemIDSkin = xmlSkinGear.attribute("itemID").as_uint();
				...
				if (itemIDSkin == itemID)
				{
					...
					for (int i=0;i<99;i++)
					{
						char buf[64];
						sprintf(buf, "sk%d", i);
						uint32_t SKINID = xmlSkinGear.attribute(buf).as_uint();
			...
			xmlSkinGear = xmlSkinGear.next_sibling();
		}
	}
	return false;
}
