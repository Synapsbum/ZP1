...
//new add skin
class GearSkinComponentConfig : public ModelItemConfig
{
public:
	GearSkinComponentConfig(uint32_t id) : ModelItemConfig(id) {}

	virtual bool loadBaseFromXml(pugi::xml_node& xmlItem);
};
//new add skin
class AirdropConfig : public ModelItemConfig
{
public:
	AirdropConfig(uint32_t id) : ModelItemConfig(id) {}
...
//new add skin
class GearSkinRecipeConfig : public ModelItemConfig
{
public:
	GearSkinRecipeConfig(uint32_t id) : ModelItemConfig(id), numComponents(0), craftedItemID(0) {}
...
public:
	...
	struct ItemsSkins
	{
		...
		ItemsSkins() {};
		ItemsSkins(uint32_t w, uint32_t s, uint32_t o) : itemSknID(w), SkinItemID(s), OriginalItemID(o) {}
	};
	ItemsSkins ItemSKN;
