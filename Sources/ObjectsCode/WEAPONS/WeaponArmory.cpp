...
bool WeaponArmory::Init()
{
	...
	{
		pugi::xml_node xmlItems = xmlDB.child("AirdropsArmory");
		pugi::xml_node xmlItem = xmlItems.child("Item");
		while(!xmlItem.empty())
		{
			loadAirdropItem(xmlItem);
			xmlItem = xmlItem.next_sibling();
		}
	}
	//new add skin
	{
		pugi::xml_node xmlItems = xmlDB.child("GearSkinComponentsArmory");
		pugi::xml_node xmlItem = xmlItems.child("Item");
		while(!xmlItem.empty())
		{
			loadGearSkinComponentItem(xmlItem);
			xmlItem = xmlItem.next_sibling();
		}
	}
	{
		pugi::xml_node xmlItems = xmlDB.child("GearSkinRecipe");
		pugi::xml_node xmlItem = xmlItems.child("Item");
		while(!xmlItem.empty())
		{
			loadGearSkinRecipeItem(xmlItem);
			xmlItem = xmlItem.next_sibling();
		}
	}
	{
		pugi::xml_node xmlItems = xmlDB.child("WeaponSkinRecipe");
		pugi::xml_node xmlItem = xmlItems.child("Item");
		while(!xmlItem.empty())
		{
...
AirdropConfig* WeaponArmory::loadAirdropItem(pugi::xml_node& xmlItem)
{
	r3d_assert(!xmlItem.empty());
	...
	uint32_t itemID = xmlItem.attribute("itemID").as_uint();
	{
		...
		if(m_itemsHash.GetObject(itemID, &temp))
		{
			r3dArtBug("Trying to load airdrop item with id '%d' that is already loaded!", itemID);
			return NULL;
		}
	}
	AirdropConfig* airdrop = game_new AirdropConfig(itemID);
	airdrop->loadBaseFromXml(xmlItem);

	m_itemsHash.Add(itemID, airdrop);
	m_NumAirdropLoaded++;

	return airdrop;
}
//new add skin
GearSkinRecipeConfig* WeaponArmory::loadGearSkinRecipeItem(pugi::xml_node& xmlItem)
{
	r3d_assert(!xmlItem.empty());
	...
	GearSkinRecipeConfig* recipe = game_new GearSkinRecipeConfig(itemID);
	recipe->loadBaseFromXml(xmlItem);

	m_itemsHash.Add(itemID, recipe);
	m_NumGearSkinRecipesLoaded++;

	return recipe;
}

GearSkinComponentConfig* WeaponArmory::loadGearSkinComponentItem(pugi::xml_node& xmlItem)
{
	r3d_assert(!xmlItem.empty());
	...
	GearSkinComponentConfig* comp = game_new GearSkinComponentConfig(itemID);
	comp->loadBaseFromXml(xmlItem);

	m_itemsHash.Add(itemID, comp);
	m_NumGearSkinComponentsLoaded++;
...
void WeaponArmory::Destroy()
{
	...
	m_NumSkinsRecipesLoaded = 0;//new add skin
	m_NumGearSkinRecipesLoaded = 0;//new add skin
	m_NumGearSkinComponentsLoaded = 0;//new add skin
	m_NumFoodItemsLoaded = 0;
	m_NumVehiclesLoaded = 0;
	m_NumItemLoaded = 0;
	m_NumGearLoaded = 0;
...
const AirdropConfig* WeaponArmory::getAirdropConfig(uint32_t itemID)
{
	BaseItemConfig* item = NULL;
	m_itemsHash.GetObject(itemID, &item);
	return (AirdropConfig*)item;
}
//new add skin
const GearSkinComponentConfig* WeaponArmory::getGearSkinComponentConfig(uint32_t itemID)
{
	BaseItemConfig* item = NULL;
	if(m_itemsHash.GetObject(itemID, &item))
	{
		if(item->category == storecat_GearSkinComponents)
			return (GearSkinComponentConfig*)item;
	}
	return NULL;
}

const GearSkinRecipeConfig* WeaponArmory::getGearSkinRecipeConfig(uint32_t itemID)
{
	BaseItemConfig* item = NULL;
	if(m_itemsHash.GetObject(itemID, &item))
	{
		if(item->category == storecat_GearSkinRecipe)
			return (GearSkinRecipeConfig*)item;
	}
	return NULL;
}
...
const SkinRecipeConfig* WeaponArmory::getSkinRecipeConfig(uint32_t itemID)
{
	...
	if(m_itemsHash.GetObject(itemID, &item))
	{
		if(item->category == storecat_WeaponsSkinsRecipe)
...
//new add skin
BackPack* WeaponArmory::createBackPack(uint32_t itemID)
{
	if (itemID == 0)
		return NULL;

	const BackpackConfig* bpk = getBackpackConfig(itemID);
	if (bpk)
		return game_new BackPack(bpk);

	r3dError("Failed to get BackPack with ID %d\nVersion mismatch!\n", itemID);
	return NULL;
}
//new add skin
