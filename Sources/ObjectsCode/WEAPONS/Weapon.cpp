...
bool isWeaponBackSlot = false;
bool isWeaponSideSlot = false;

r3dMesh* WeaponAttachmentConfig::getMesh(bool allow_async_loading, bool aim_model, bool IsReloading, bool IsEquiped, int BulletsInClip) const
{
	// Don't draw the magazine if we are reloading
	if (IsReloading && m_type == WPN_ATTM_CLIP)
		return NULL;

	// fix flying ammo at the shotguns
	if (m_itemID == 400136 || m_itemID == 400141 || m_itemID == 400142 || m_itemID == 400152 || m_itemID == 400165 || m_itemID == 400166 || m_itemID == 400046 || m_itemID == 400145)
		return NULL;
...
{
	if (isWeaponBackSlot && !first_person || isWeaponSideSlot && !first_person || category == storecat_UsableItem && !first_person || category == storecat_GRENADE && !first_person /*|| category == storecat_MELEE && !first_person*/)
	{
		m_Model = r3dGOBAddMesh(m_ModelPath, true, false, allow_async_loading, true);
		if (m_Model == 0)
		{
...
, ActualSkin(0)//new add skin
{
	...
	//AlexRedd:: disabled for now! it might be a reason why ppls crashed with skeleton error
...
Weapon::~Weapon()
{
	...
	/*if (m_ShellExtractParticle)//AlexRedd:: disabled for now! it might be a reason why ppls crashed with skeleton error
...
void Weapon::OnEquip(bool isFlashlightOn)
{
	...
	const WeaponAttachmentConfig* clipCfg = getClipConfig(); // Reload if gun is empty
	if (clipCfg)
	{
		// to make sure the player is not in the main menu
		if (gClientLogic().localPlayer_) {
			if (getNumBulletsLeft() <= 0 && gClientLogic().localPlayer_->PlayerState != PLAYER_MOVE_SPRINT && gClientLogic().localPlayer_->bDead != 0)
			{
...
void Weapon::OnUnequip()
{
	...
	if (m_sndReload)
	{
		SoundSys.Stop(m_sndReload);
		SoundSys.Release(m_sndReload);
		m_sndReload = 0;
	}
...
void Weapon::switchFiremode()
{
	...
	if (m_firemode != prevFiremode) // switched firemode, reset weapon
	{
		...
		if(gClientLogic().localPlayer_)
			SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/Misc/Weapon_FireMode_Change"), gClientLogic().localPlayer_->GetPosition());
...
#pragma omp parallel /*for*/ num_threads(omp_get_num_threads())
	...
	if (m_State == WPN_RELOADING)
	{
		...
		if (getReloadProgress() > m_ReloadTime) // done reloading
		{
			...
			if (!clipCfg)
			{
				r3dOutToLog("!!! weapon can't be reloaded - there is no clip defined #1\n");
				return;
			}

#ifndef FINAL_BUILD // STUDIO RELOAD
			extern bool g_bEditMode;
			if (g_bEditMode)
			{
			...
			if (m_Owner->NetworkLocal)
			{
				...
				if (ammoSlot == -1)
				{
					...
					for (int i = 0; i < m_Owner->CurLoadout.BackpackSize; i++)
					{
						if (m_Owner->CurLoadout.Items[i].itemID == clipCfg->m_itemID)
						{
							wiInventoryItem& ammoItm = m_Owner->CurLoadout.Items[i];
...
r3dMesh* Weapon::getMeshSkin(bool isFirstPerson, bool m_ForceTPSModel)
{
	if (this == NULL)
		return NULL;
	...
	{
		isFirstPerson = !m_ForceTPSModel; //fix
		m_pConfig->ForceTPSWeapon = m_ForceTPSModel;
	}
			...
			if (itemID != 0)
			{
				...
				if (itemIDSkin == itemID)
				{
					...
					for (int i = 0; i<99; i++)
					{
						...
						//	r3dOutToLog("##### WeaponID %i SKINS %d\n",itemIDSkin,SKINID);
...
r3dPoint3D Weapon::getMuzzlePos2(const D3DXMATRIX& weaponBone) const
{
	D3DXVECTOR3 res;
	D3DXVECTOR3 offset = m_pConfig->muzzlerOffset;
	D3DXVec3TransformNormal(&res, (D3DXVECTOR3*)&offset, &weaponBone);
	r3dPoint3D muzzlerPoint = r3dPoint3D(weaponBone._41, weaponBone._42, weaponBone._43) + r3dPoint3D(res.x - 1.0f, res.y, res.z); // 1.5 should be fine
	// PT: sometimes animation has QNAN in it, not sure where is it coming from. Seems like in some cases quaternion becomes fucked up and not able to transform it into a matrix. As all other data in skeleton is fine
	if (!(r3d_float_isFinite(muzzlerPoint.x) && r3d_float_isFinite(muzzlerPoint.y) && r3d_float_isFinite(muzzlerPoint.z)))
		muzzlerPoint.Assign(0, 0, 0);
	return muzzlerPoint;
...
int Weapon::getNumShotsRequired()
{
	...
	if (m_pConfig->category == storecat_GRENADE || m_pConfig->category == storecat_SUPPORT)
		return numShots;
...
#pragma omp parallel /*for*/ num_threads(omp_get_num_threads())
	...
	else
	{
		...
		// add shell
		//r3dPoint3D shellPos = getShellPos(weaponBone);//AlexRedd:: disabled for now! it might be a reason why ppls crashed with skeleton error
		r3dPoint3D shellDir = getShellDir(weaponBone);
		if (m_pConfig->category != storecat_SUPPORT && m_pConfig->category != storecat_MELEE && m_pConfig->category != storecat_GRENADE && distToLocalPlayer < 30.0f && executeWeaponFireCode && m_pConfig->m_itemID != 101322 && m_pConfig->m_itemID != 101392) //101322 - crossbow, 101392 - nailgun
		{
...
//#if !R3D_NO_BULLET_MGR
				//if (m_Owner == localPlayer)
				//GameWorld().m_BulletMngr->AddShell(shellPos, shellDir*u_GetRandom(0.15f, 0.25f), shellOrient, shellType, true);
				//else
					//GameWorld().m_BulletMngr->AddShell(shellPos + r3dPoint3D(0, -1.5, 0), shellDir*u_GetRandom(0.15f, 0.25f), shellOrient, shellType, false);
//#endif
			}
		}
		...
		if (m_MuzzleParticle && distToLocalPlayer < 1000.0f && playerVisible && executeWeaponFireCode)
		{
			if (!localPlayerInSniperMode)
			{
				...
				if (m_Owner != localPlayer)
					m_MuzzleParticle->SetPosition(muzzlerPoint + r3dPoint3D(0, -1.5, 0));
				else
					m_MuzzleParticle->SetPosition(muzzlerPoint);
...
void Weapon::Reload(int ammoslot)
{
	...
	if (!clipCfg)
	{
		r3dOutToLog("!!! weapon can't be reloaded - there is no clip defined #2\n");
		return;
	}
	...
	// if we don't own a clip anymore
	extern bool g_bEditMode;
	if (!g_bEditMode && getNumClipsLeft() == 0)
		return;
	...
	optionalAmmoSlotToUse = ammoslot;

	m_Owner->StartReload(m_BackpackIdx);
	StartReloadSequence();
}
...
bool Weapon::isWeaponBack(bool show) const
{
	return isWeaponBackSlot = show;
}

bool Weapon::isWeaponSide(bool show) const
{
	return isWeaponSideSlot = show;
}
...
void Weapon::setWeaponAttachments(const WeaponAttachmentConfig** wpnAttmConfigs)
{
	...
	for (int i = 0; i < WPN_ATTM_MAX; ++i)
	{
		...
		if (m_Attachments[i] == wpnAttmConfig) // if that attachment already installed - then skip
			continue;
		...
		if (wpnAttmConfig)
		{
			wpnAttmConfig->getMesh(true, false, isReloading(), m_IsEquiped, getNumBulletsLeft());
			wpnAttmConfig->getMesh(true, true, isReloading(), m_IsEquiped, getNumBulletsLeft());
		}
...
r3dMesh* Weapon::getWeaponAttachmentMesh(WeaponAttachmentTypeEnum attm_type, bool aim_model)
{
	if (m_Attachments[(int)attm_type])
	{
		return m_Attachments[(int)attm_type]->getMesh(true, aim_model, isReloading(), m_IsEquiped, getNumBulletsLeft());
	}
