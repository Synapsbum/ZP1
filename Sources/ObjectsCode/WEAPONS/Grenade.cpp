...
BOOL obj_Grenade::OnCreate()
{
	...
	{
		char tmpStr[16];		
		if (m_Weapon->m_itemID == WeaponConfig::ITEMID_FragGrenade)			
			sprintf(tmpStr, "grenade");
		else if (m_Weapon->m_itemID == WeaponConfig::ITEMID_FlashBang)
			sprintf(tmpStr, "flash");
		else if (m_Weapon->m_itemID == WeaponConfig::ITEMID_SmokeGrenadeWhite)
			sprintf(tmpStr, "smoke");
		hudMain->addScreenIcon(m_GrenadeIcon, tmpStr);
	}
...
BOOL obj_Grenade::OnDestroy()
{
	...
	if (hudMain && !m_GrenadeIcon.IsUndefined())
		hudMain->removeUserIcon(m_GrenadeIcon);
...
BOOL obj_Grenade::Update()
{
...

	{
		...
		if (hudMain && !m_GrenadeIcon.IsUndefined())
		{
			hudMain->moveUserIcon(m_GrenadeIcon, GetPosition(), true);
			hudMain->setScreenIconAlpha(m_GrenadeIcon, alpha);
			hudMain->setScreenIconDistance(m_GrenadeIcon, GetPosition());
			hudMain->setScreenIconScale(m_GrenadeIcon, R3D_CLAMP(alpha, 0.5f, 1.0f));
		}
	}
