...
public:
	...
	r3dMesh*	getMeshSkin(bool isFirstPerson, bool m_ForceTPSModel = false);//new add skin
	...
	r3dPoint3D getMuzzlePos2(const D3DXMATRIX& weaponBone) const;
	const char* getMuzzlePartilce() const { return m_pConfig->m_MuzzleParticle; }

	//r3dPoint3D getShellPos(const D3DXMATRIX& weaponBone) const;//AlexRedd:: disabled for now! it might be a reason why ppls crashed with skeleton error
	...
	bool	isWeaponBack(bool show) const;
	bool	isWeaponSide(bool show) const;
...
private:
	...
	//class obj_ParticleSystem* m_ShellExtractParticle;//AlexRedd:: disabled for now! it might be a reason why ppls crashed with skeleton error
	int m_firemode;
