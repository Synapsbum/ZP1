...
Ammo::Ammo(const char* ammoName)
{
	...
	m_ModelPath = NULL;
	m_ParticleTracer = 0;
	//m_ShellExtractParticle = 0;//AlexRedd:: disabled for now! it might be a reason why ppls crashed with skeleton error
	m_DecalSource = NULL;
	m_BulletClass = 0;
}
...
Ammo::~Ammo()
{
	...
	//free(m_ShellExtractParticle);//AlexRedd:: disabled for now! it might be a reason why ppls crashed with skeleton error
	free(m_DecalSource);
	free(m_BulletClass);
}
