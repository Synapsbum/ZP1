...
const uint32_t			g_NumZombieTypes				= 4;
r3dSkeleton*	g_zombieBindSkeleton[g_NumZombieTypes]  = { NULL, NULL, NULL, NULL };
r3dAnimPool*	g_zombieAnimPool[g_NumZombieTypes]		= { NULL, NULL, NULL, NULL };
...
static const char* GetZombiePartName(int type, int slot, int idx)
{
	...
	else			// Normal Zombie
	{
		sprintf(buf, "%s\\Zombie_%s_%02d.sco",
			"Data\\ObjectsDepot\\WZ_Zombie_Test",
			names[slot], 
			idx);
	}		
	return buf;
...
obj_ZombieDummy::obj_ZombieDummy()
{
	...
	if(g_zombieBindSkeleton[0] == NULL) 
	{
		...
		g_zombieBindSkeleton[0]->LoadBinary("Data\\ObjectsDepot\\WZ_Zombie_Test\\ProperScale_AndBiped.skl");
	...
	{
		g_zombieBindSkeleton[2] = game_new r3dSkeleton();
		g_zombieBindSkeleton[2]->LoadBinary("Data\\ObjectsDepot\\ZH_Characters_Zombies\\Super_Zombie_Berserker_Body_01.skl");
	...
	m_typeIndex = u_random(2);//u_random(g_NumZombieTypes);
...
obj_ZombieDummy::~obj_ZombieDummy()
{
	if(--g_zombieInstances == 0)
	{
		...
		SAFE_DELETE(g_zombieAnimPool[2]);
		SAFE_DELETE(g_zombieBindSkeleton[2]);
	}
}
...
BOOL obj_ZombieDummy::Update()
{
	...
	if(WalkSpeed > 0.01) 
	{
		...
		if(pos.z < SpawnPos.z - WalkDistance) pos.z = SpawnPos.z + WalkDistance;
		SetPosition(pos);
	}
...
float obj_ZombieDummy::DrawPropertyEditor(float scrx, float scry, float scrw, float scrh, const AClass* startClass, const GameObjects& selected)
{
	...
	starty += imgui_Value_Slider(scrx, starty, "WalkDistance", &WalkDistance, 2, 20, "%f");
	...
	starty += imgui_Static(scrx, starty, " Select Animation");
	{
		...
		if (m_isSuperZombie)
			...
			sDirFind = "Data\\ObjectsDepot\\WZ_Zombie_Test\\Animations\\*.anm";
