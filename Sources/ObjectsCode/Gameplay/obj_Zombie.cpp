...
extern r3dSkeleton*	g_zombieBindSkeleton[4];
extern r3dAnimPool*	g_zombieAnimPool[4];
...
#endif
{
	...
	if (g_zombieBindSkeleton[2] == NULL)
	{
		...
		g_zombieBindSkeleton[2]->LoadBinary("Data\\ObjectsDepot\\ZH_Characters_Zombies\\Super_Zombie_Mutant_body_01.skl");

		g_zombieAnimPool[2] = game_new r3dAnimPool();
	}

	if (g_zombieBindSkeleton[3] == NULL)
	{
		g_zombieBindSkeleton[3] = game_new r3dSkeleton();
		g_zombieBindSkeleton[3]->LoadBinary("Data\\ObjectsDepot\\ZH_Characters_Zombies\\Super_Zombie_Berserker_Body_01.skl");

		g_zombieAnimPool[3] = game_new r3dAnimPool();
	}
...
BOOL obj_Zombie::OnCreate()
{
	...
	switch( CreateParams.HeroItemID )
	{
		case g_SuperZombieItemID:
			m_typeIndex = 1;
			break;
		case g_SuperZombieMutantItemID:
			m_typeIndex = 2;
			break;
		case g_SuperZombieBerserkerItemID:
			m_typeIndex = 3;
			break;
		default:
			m_typeIndex = 0;
	...
	if (CreateParams.HeroItemID >= 20249 && CreateParams.HeroItemID <= 20255 || CreateParams.HeroItemID == g_SuperZombieMutantItemID || CreateParams.HeroItemID == g_SuperZombieBerserkerItemID)
	...
	//AlexRedd:: halloween zombies :)
	if(CreateParams.HalloweenZombie)
	{		
		const GearConfig* gc = g_pWeaponArmory->getGearConfig(CreateParams.HalloweenZombieItemID);
		if(gc)
			zombieParts[3] = r3dGOBAddMesh(gc->m_ModelPath, true, false, true, true );	
	}
	...
	ZombieAnimGroup = (!IsSuperZombie() && !IsSuperZombieMutant() && !IsSuperZombieBerserker() && CreateParams.FastZombie) ? (1 + u_random(2)) : 0;

	UpdateAnimations( 0 );

	m_isFemale = CreateParams.isFemale;
...
{
	if (gClientLogic().localPlayer_ == NULL || bDead || !isSoundAudible() || !IsSuperZombie() && !IsSuperZombieMutant() && !IsSuperZombieBerserker())
		return;
	...
	case EZombieStates::ZFidget_SZIdleAlertSniff:
		...
		break;
	case EZombieStates::ZFidget_SZIdleAlertAgr:
...
void obj_Zombie::UpdateConcurrent( int fullAnimUpdate )
{
	...
	if( g_enable_zombie_sprint->GetBool() ) // Turn on/off Sprinting
	{
			...
			( 0.0f < SprintTimer || 0.0f < SprintFalloffTimer ) )
		{
			...
			if (IsSuperZombie() || IsSuperZombieMutant() || IsSuperZombieBerserker() || !staggeredTrack)
			{
	...
	if ((IsSuperZombie() || IsSuperZombieMutant() || IsSuperZombieBerserker() || !staggeredTrack) && (EZombieStates::ZState_Walk == ZombieAnimState || EZombieStates::ZState_Pursue == ZombieAnimState || EZombieStates::ZState_PursueSprint == ZombieAnimState))
	{
	...
	else if ((IsSuperZombie() || IsSuperZombieMutant() || IsSuperZombieBerserker() || !staggeredTrack) && EZombieStates::ZState_Attack == ZombieAnimState)
	{
		if ((IsSuperZombie() || IsSuperZombieMutant() || IsSuperZombieBerserker()) && attackTrack)
		{
...
{
	if (!IsSuperZombie() && !IsSuperZombieMutant() && !IsSuperZombieBerserker() || (ZombieAnimState == EZombieStates::ZState_Walk && ZombieAnimStateLevel == 1))
		return;	
...
{
	if (!IsSuperZombie() && !IsSuperZombieMutant() && !IsSuperZombieBerserker() || (ZombieAnimState == EZombieStates::ZState_Pursue && ZombieAnimStateLevel == 1))
		return;	
...
{
	if (!IsSuperZombie() && !IsSuperZombieMutant() && !IsSuperZombieBerserker() || (ZombieAnimState == EZombieStates::ZState_PursueSprint && ZombieAnimStateLevel == 1))
		return;	
...
int obj_Zombie::PlayStaggeredAnim()
{
	...
	if (IsSuperZombie() || IsSuperZombieMutant() || IsSuperZombieBerserker())
	{
		...
		enableAnimBones("Bip01_Spine1", g_zombieBindSkeleton[ m_typeIndex ], ad, true);
...
int obj_Zombie::PlaySuperZombieAlertAnim(int alert)
{
	...
	if (IsSuperZombie() || IsSuperZombieMutant() || IsSuperZombieBerserker())
	{
		...
		case EZombieStates::ZFidget_SZIdleAlertSniff:
			...
			break;
		case EZombieStates::ZFidget_SZIdleAlertAgr:
			aid = AddAnimation("Super_Zombie_Alert_to_Run");
			break;
		};
	...
	if (IsSuperZombie() || IsSuperZombieMutant() || IsSuperZombieBerserker())
	{
		...
		case EZombieStates::ZFidget_SZIdleAlertAgr:			
			...
			if (IsSuperZombieMutant())
				SpawnImpactParticle(r3dHash::MakeHash(""), r3dHash::MakeHash("SuperMutant"), from->GetPosition(), r3dPoint3D(0,1,0));			
			break;
...
{
	if (vehicle->GetSpeed() < 1.0f || ZombieState == EZombieStates::ZState_FakeDead || IsSuperZombie() || IsSuperZombieMutant() || IsSuperZombieBerserker())
		return;
