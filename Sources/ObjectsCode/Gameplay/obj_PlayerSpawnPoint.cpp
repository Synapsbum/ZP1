...
{
	const char* cpMeshName = "Data\\ObjectsDepot\\ZH_Objects_Editor\\Player_Spawn_Point_01.sco";
...
struct PlaceableConsumableItemRenderable : MeshDeferredRenderable
{
	...
	static void Draw(Renderable* RThis, const r3dCamera& Cam)
	{
		...
		r3dApplyPreparedMeshVSConsts(This->Parent->MeshVSConsts);
...
void obj_PlayerSpawnPoint::DoDrawComposite( const r3dCamera& Cam )
{
	...
	if(spawnType_ == SPAWN_TELEPORT) clr = r3dColor(77,253,6);
	
	r3dRenderer->SetRenderingMode(R3D_BLEND_NZ | R3D_BLEND_PUSH);
	
	r3dDrawLine3D(GetPosition() + r3dPoint3D(0, 2.0f, 0), GetPosition() + r3dPoint3D(0, 20.0f, 0), Cam, 0.4f, clr);
	...
	for(int i=0; i<m_NumSpawnPoints; ++i)
	{
		...
		else if(spawnType_ == SPAWN_TELEPORT)
			r3dDrawCircle3D(m_SpawnPoints[i].pos, 2.0f, Cam, 0.4f, ((i==m_SelectedSpawnPoint&&g_Manipulator3d.PickedObject() == this)?r3dColor24::red:r3dColor24::green));
		else
...
float obj_PlayerSpawnPoint::DrawPropertyEditor(float scrx, float scry, float scrw, float scrh, const AClass* startClass, const GameObjects& selected)
{
	...
	{		
		...
		if( !names.size() )
		{
			...
			names.push_back( "TELEPORT (Used for Teleport)" );
		}
		...
		if( selected.Count() <= 1 )
		{
			...
			// Repos spawn location by click combi
			if((Mouse->WasPressed(r3dMouse::mLeftButton)) && Keyboard->IsPressed(kbsLeftShift) && UI_TargetObjID != this->ID)
				m_SpawnPoints[m_SelectedSpawnPoint].pos = UI_TargetPos + r3dPoint3D(0.0f, 0.1f, 0.0f);

			// Add new spawn location by click combi
			if ((Mouse->WasPressed(r3dMouse::mLeftButton)) && Keyboard->IsPressed(kbsLeftControl) && UI_TargetObjID != this->ID)
			{
				if (m_NumSpawnPoints < MAX_SPAWN_POINTS)
				{
					m_SpawnPoints[m_NumSpawnPoints].pos = UI_TargetPos + r3dPoint3D(0.0f, 0.1f, 0.0f);
					m_SpawnPoints[m_NumSpawnPoints].dir = 0;
					m_SelectedSpawnPoint = m_NumSpawnPoints;
					m_NumSpawnPoints++;
				}
			...
			starty += imgui_Static(scrx, starty, "Tip: You can repos spawn locations by pressing LEFT-SHIFT + LEFT-MOUSEBUTTON");
			starty += imgui_Static(scrx, starty, "Tip: You can add a new spawn location by pressing LEFT-CONTROL + LEFT-MOUSEBUTTON");
		}
	}
