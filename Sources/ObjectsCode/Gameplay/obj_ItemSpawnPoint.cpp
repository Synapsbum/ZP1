...
bool isSpeciallootBoxes(int ItemId)// filter special id's (unneeded lotboxes for spawns)
{
	bool specialItem = false;	

	switch (ItemId)
	{
	case 301410:// MISSIONS - Secret of key Loot
		specialItem = true;
		break;
	case 301432:// BATTLE ROYALE - ITEM REWARDS
		specialItem = true;
		break;
	case 301427:// INGAME CASES
	case 301428:
	case 301429:
	case 301430:
		specialItem = true;
		break;
	case 301415:// MYSTERY CASES
	case 301416:
	case 301417:		
		specialItem = true;
		break;
	case 301419:// AIRDROP LOOTS
	case 301420:
	case 301421:
	case 301422:
	case 301423:
...
float obj_ItemSpawnPoint::DrawPropertyEditor(float scrx, float scry, float scrw, float scrh, const AClass* startClass, const GameObjects& selected)
{
	...
	{	
		...
		if(!m_OneItemSpawn)
		{
			...
			if(numLootBoxes == 0)
			{
				...
				while(g_pWeaponArmory->searchNextItem())
				{
					...
					if (cfg->category == storecat_LootBox && !isSpeciallootBoxes(itemID))
					{
						tempS holder;
		...
		if (selected.Count() <= 1)
		{
			...
			if ((Mouse->WasPressed(r3dMouse::mLeftButton)) && Keyboard->IsPressed(kbsLeftShift) && !m_SpawnPointsV.empty() && UI_TargetObjID != this->ID)
			...
			starty += imgui_Static(scrx, starty, "Tip: Repos locations LEFTSHIFT+LEFTMOUSEBUTTON");
			starty += imgui_Static(scrx, starty, "Tip: Add spawn location LEFTCONTROL+LEFTMOUSEBUTTON");
		}
	}
...
void obj_ItemSpawnPoint::AnalyzeLootBoxData(float analyzeWindowInHours)
{
	// firstly update loot box content
	{
		...
		CkHttpRequest req;
		req.UsePost();
		req.put_Path("/ZP/api/php/api_GetLootBoxConfig.php");
