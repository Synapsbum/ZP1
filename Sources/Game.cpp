...
#pragma omp parallel /*for*/ num_threads(omp_get_num_threads())
	...
	if (!g_increase_fps->GetBool())//Increase FPS option..	
	...
	if (!g_bEditMode)
	{
		HudArray[TypesofHUD::T_TPSGameHUD] = game_new TPSGameHUD;
	}
	...
	if (g_bEditMode)
	{
		HudArray[TypesofHUD::T_CameraHUD] = game_new CameraHUD;
		HudArray[TypesofHUD::T_EditorGameHUD] = game_new EditorGameHUD;
		HudArray[TypesofHUD::T_ParticleHUD] = game_new ParticleHUD;
		HudArray[TypesofHUD::T_PhysicsHUD] = game_new PhysicsHUD;
		HudArray[TypesofHUD::T_CharacterHUD] = game_new CharacterHUD;
	}
#endif
...
static void LoadGameWithLoadScreen(const char* LevelFolder )
{
	...
	DoLoadingScreen(&gGameLoadActive, LevelFolder, (g_bEditMode ? "EDITOR" : gClientLogic().m_gameInfo.name), -1);
...
void GameStateGameLoop()
{
	...
	if( g_LoadToStartOutputDone < 3 )
	{
		...
		if( g_LoadToStartOutputDone == 3 )
		{
			r3dOutToLog( "Time between load end and game start: %.2f\n", r3dGetTime() - g_LoadLevelTimeMark );
		}
	}
	...
	//AlexRedd:: winter for colorado v2 map	
	//if(!g_bEditMode)
		//r_show_winter->SetInt(gClientLogic().m_gameInfo.mapId == GBGameInfo::MAPID_ZP_Test?true:false);	
	...
	if (hudPause && hudPause->needUpdateMode_)
	{
		r3dRenderer->UpdateMode();
		hudPause->needUpdateMode_ = false;
	}
}
...
static bool BackToAppSelect = false;
...
void PlayEditor()
{
	...
	if (g_bStartedAsParticleEditor)
		SetHud(3);
	else if (g_bStartedAsPhysXEditor)
		SetHud(4);
	else if (g_bStartedAsCharacterEditor)
		SetHud(5);
	else
		SetHud(1);
	...
	while (CurrentGameMode != GAMESTATE_EXIT)
	{
		...
		if (Keyboard->WasPressed(kbsF1) || Keyboard->WasPressed(kbsEsc)) // level
		{
			g_bStartedAsParticleEditor = false;
			SetHud(1);
		}
		if (Keyboard->WasPressed(kbsF2)) // Particle
		{
			if (!g_bStartedAsParticleEditor)
		...
		if (Keyboard->WasPressed(kbsF3)) // physx
		{
			g_bStartedAsParticleEditor = false;
			SetHud(4);
		}
		if (Keyboard->WasPressed(kbsF4)) // character
		{
			g_bStartedAsParticleEditor = false;
			SetHud(2);
		}
		if (Keyboard->WasPressed(kbsF5)) // character EDITOR
		{
			g_bStartedAsParticleEditor = false;
			SetHud(5);
		}
		...
		if (LevelEditor.leave)
		{
			// request loop close
			CurrentGameMode = GAMESTATE_EXIT;
			BackToAppSelect = true;
		}
	} // Frame loop
