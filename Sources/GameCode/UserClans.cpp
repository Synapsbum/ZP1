...
void CUserClans::ParseClanInfo(pugi::xml_node& xmlNode, ClanInfo_s& clan)
{
	...
	clan.FriendlyFire    = xmlNode.attribute("FFire").as_int();//AlexRedd:: FriendlyFire system
	clan.ClanPoints		 = xmlNode.attribute("ClanPoints").as_int();//AlexRedd:: clan points system
...
void CUserClans::ParseClanMember(pugi::xml_node& xmlNode, ClanMember_s& member)
{
	...
	member.stats.VictorysBattleRoyale   = xmlNode.attribute("ts03").as_uint(); //AlexRedd:: BR mode
...
void CUserClans::ParseClanApplications(pugi::xml_node& xmlNode)
{
	...
	while(!xmlApply.empty())
	{
		...
		capply.stats.VictorysBattleRoyale   = xmlApply.attribute("ts03").as_uint(); //AlexRedd:: BR mode
...
int CUserClans::ApiSetFriendlyFire(int ClanID, int value)
{
	CWOBackendReq req(&gUserProfile, "api_ClanMgr.aspx");
	req.AddParam("func",     "friendlyfire");
	req.AddParam("ClanID", ClanID);
	req.AddParam("friendfire", value);

	if(!req.Issue())
	{
		r3dOutToLog("ApiSetFriendlyFire FAILED, code: %d\n", req.resultCode_);
		return req.resultCode_;
	}
...
//AlexRedd:: Clan points
int CUserClans::ApiSetClanPoints(int ClanID, int points)
{
	CWOBackendReq req(&gUserProfile, "api_ClanMgr.aspx");
	req.AddParam("func",     "setpoints");
	req.AddParam("ClanID", ClanID);
	req.AddParam("points", points);

	if(!req.Issue())
	{
		r3dOutToLog("ApiSetClanPoints FAILED, code: %d\n", req.resultCode_);
		return req.resultCode_;
	}
