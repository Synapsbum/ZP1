...
void HUDActionUI::setCarInfo(int durability, int speed, int armor, int weight, int gas, bool hasOwner, const char* ownername, int isLocked, bool isOwner, const char* lockText)
{
	if(!isInit) return;
	...
	var[5].SetBoolean(hasOwner);
	var[6].SetString(ownername!=""?ownername:"");
	var[7].SetInt(isLocked);
	var[8].SetBoolean(isOwner);
	var[9].SetString(lockText);
	gfxMovie.Invoke("_root.api.Main.setCarInfo", var, 10);	
}
