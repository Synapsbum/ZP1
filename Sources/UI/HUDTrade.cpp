...
void HUDTrade::eventBackpackToTrade(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	//AlexRedd:: no drop items
	if(/*wi1.itemID == WeaponConfig::ITEMID_NoDrop_Weapon ||*/ wi1.itemID == WeaponConfig::ITEMID_NoDrop_Backpack)
	{
		showForbidden();
		return;
	}
...
void HUDTrade::showForbidden()
{
	Scaleform::GFx::Value var[3];
	var[0].SetString(gLangMngr.getString("TradeItemForbidden"));
	var[1].SetBoolean(true);
	var[2].SetString("");
	gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
