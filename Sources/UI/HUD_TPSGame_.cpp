...
void TPSGameHUD :: SetCameraPure ( r3dCamera &Cam)
{
	...
	if(pl->bDead)
	{
		obj_Player* killer = (obj_Player*)GameWorld().GetNetworkObject(pl->Dead_KillerID);
		//GameObject* killedByObj = GameWorld().GetNetworkObject(pl->Dead_KillerID);
		...
		if((r3dGetTime() - pl->TimeOfDeath)<3.0f) // show dead player for 2 seconds
		{
			...
			if(!oldPlayerPos.AlmostEqual(pl->GetPosition())) // make sure to do that check only once
			{
				...
				{
					camPosOffset = r3dPoint3D(0.0f, 0, 0.0f);
					//camPosOffset = r3dPoint3D(-0.1f, 5, -0.1f);//WarInc
				}
			}
		...
		//else if (killedByObj->isObjType(OBJTYPE_Zombie))//By zombie
		//{

		//}
		else if(killer && (r3dGetTime() - pl->TimeOfDeath)<4.0f && !pl->DisableKillerView && CGL.m_gameInfo.channel != 6) // for next 2 seconds show enemy
		{
		...
		/*else if(CGL.m_gameInfo.channel == 6 && (r3dGetTime() - pl->TimeOfDeath)>=2.0f && !hudPause->isActive()) // after that in bomb mode show your allies until respawn time
		{
			//hudMain->switchToDead(true, true); // force spectator mode
			static int friendlyIndx = 0;
			bool find_new_ally = false;
			obj_Player* ally = CGL.GetPlayer(friendlyIndx);
			if(ally == NULL) 
				find_new_ally = true;
			else if(ally->bDead && (r3dGetTime()-ally->TimeOfDeath)>5.0f) // allow X seconds to look at dead body
				find_new_ally = true;
			else if(ally->ClanID != CGL.localPlayer_->ClanID)
				find_new_ally = true;
			...
			if(find_new_ally)
			{
				...
				for(int i=friendlyIndx+1; i<CGL.CurMaxPlayerIdx; ++i)
				{
					...
					if(plr && !plr->bDead && 
						plr->ClanID == CGL.localPlayer_->ClanID && 
				...
				if(!foundPlr)
				{
					for(int i=0; i<CGL.CurMaxPlayerIdx; ++i)
					{
						obj_Player* plr = CGL.GetPlayer(i);
						if(plr && !plr->bDead && 
							plr->GroupID == CGL.localPlayer_->GroupID && 
							plr != CGL.localPlayer_)
						{
							friendlyIndx = i;
							ally = plr;
...
static void DrawMenus()
{
	...
	*/
	...
	if(!win::bSuspended && !hudMain->isChatInputActive() && !hudMain->isPlayersListVisible()) 
	{
		...
		if(showHudPause && !hudAttm->isActive() && !hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudCraft->isActive() && !hudSell->isActive())
		{
		...
		if(showCraft && !hudAttm->isActive() && !hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudPause->isActive() && !hudSell->isActive())
		{
		...
		if( pl && !hudCraft->isActive() && !hudAttm->isActive() && !hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudPause->isActive() && !hudSell->isActive() )
		{
...
void ProcessPlayerMovement(obj_Player* pl, bool editor_debug )
{
	...
	if(!pl->m_isInScope)
	{
		if(InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_SWITCH_TPS_CAMERA_SIDE) && !(hudVault && hudVault->isActive()) && 
			...
			!(hudSafelock && hudSafelock->isActive()) && !(hudSell && hudSell->isActive()))//cynthia: 1785
		{
