...
void PhysicsHUD::Process()
{
	...
	float   glb_MouseSensAdj = r_spectator_mouse_sens->GetFloat();	// in range (0.01 - 1.0) //AlexRedd:: spectator mouse sensitivity

	static int old_enablemouselook = false;
	old_enablemouselook = enablemouselook;
	...
	if (Keyboard->IsPressed(kbsS)) FPS_Acceleration.Z = -fSpeed;
	if (Keyboard->IsPressed(kbsA)) FPS_Acceleration.X = -fSpeed;
	if (Keyboard->IsPressed(kbsD)) FPS_Acceleration.X = fSpeed;
	...
	if (r_editor_use_lerp->GetBool())
	{
		static float xMult = 0.0f;
		static float zMult = 0.0f;
		if (!old_enablemouselook)
		{
			// reset
			xMult = 0.0f;
			zMult = 0.0f;
		...
		xMult = R3D_LERP(xMult, FPS_Acceleration.X * r3dGetFrameTime() * mult, r3dGetFrameTime() * r_editor_lerp_speed->GetFloat());
		zMult = R3D_LERP(zMult, FPS_Acceleration.Z * r3dGetFrameTime() * mult, r3dGetFrameTime() * r_editor_lerp_speed->GetFloat());
		FPS_Position += FPS_vVision * zMult;
		FPS_Position += FPS_vRight * xMult;
	}
	else
	{
...
void PhysicsHUD::ProcessPhysicsEditor()
{
	...
	SliderY += imgui_Checkbox((float)SliderX, (float)SliderY, "Draw Collision Meshes",	    &g_DrawCollisionMeshes, 1);
	extern int g_DrawPlayerOnlyCollisionMeshes;
	SliderY += imgui_Checkbox((float)SliderX, (float)SliderY, "Draw Player Only Collision Meshes",	    &g_DrawPlayerOnlyCollisionMeshes, 1);
#ifndef FINAL_BUILD
	int prevDrawPhysicsDebug = d_physx_debug->GetInt();	
	SliderY += imgui_Checkbox((float)SliderX, (float)SliderY, "Enable Physics Debug",	    &DrawPhysicsDebug, 1);
	if(prevDrawPhysicsDebug != DrawPhysicsDebug)
	{
