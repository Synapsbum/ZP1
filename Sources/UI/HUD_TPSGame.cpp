...
#include "UI\HUDSkins.h"//new add skin
...
HUDSkins* hudskns = NULL;//new add skin
...
void TPSGameHUD_OnStartGame()
{
	...
	hudCommCalls = game_new HUDCommCalls();
	hudskns = game_new HUDSkins();//new add skin
...
void TPSGameHUD :: DestroyPure()
{
	if(TPSGameHud_Inited)
	{
		...
		SAFE_DELETE(hudskns);//new add skin
	}
}
...
void ApplyGunShotShake(r3dPoint3D& camUp, float shakeIntensity) //ZP: 20/06/2024 - Silah Gun Shot Shake CodeX
{
	static float shakeTimer = 0.0f;
	shakeTimer += r3dGetFrameTime();

	// Shake effect using a sine wave
	float shakeAmount = sin(shakeTimer * 50.0f) * shakeIntensity;

	// Apply the shake to the camera's up vector
	r3dPoint3D shakeVector(0, shakeAmount, 0);
	camUp += shakeVector;
	camUp.Normalize();

	// Reduce shake intensity over time
	shakeIntensity = R3D_MAX(shakeIntensity - r3dGetFrameTime() * 5.0f, 0.0f);
}
...
void Get_Camera_Bob(r3dPoint3D& camBob, r3dPoint3D& camUp, const obj_Player* player)
{
	...
	accumul += r3dGetFrameTime()*1.0f*u_GetRandom(0.45f, 1.02f);
	...
	{
		g_shootCameraShakeTimer = R3D_MAX(g_shootCameraShakeTimer-r3dGetFrameTime()*2.0f, 0.0f);
	}
	...
	//{	
		...
		up.RotateAroundVector(rightVector, wave*12.0f );
		...
		case PLAYER_MOVE_RUN:
			{
				boba *= .3f; // yeah this doesn't do anything, it's here in case we want to tweak it. 
				rolla = 1;
			}
			break;
...
void		updateCameraLeftSide()
{
	if(g_CameraLeftSideLerp < 1.0f)
		g_CameraLeftSideLerp = R3D_CLAMP(g_CameraLeftSideLerp+r3dGetFrameTime()*4.0f, 0.0f, 1.0f);
}
...
r3dPoint3D getAdjustedPointTo(obj_Player* pl, const r3dPoint3D& PointTo, const r3dPoint3D& CamPos)
{
	...
	else if(!g_CameraPointToAdj_HasAdjustedVec)
	{
        ...
        r3dPoint3D playerPosHead = pl->GetPosition(); 
		playerPosHead.y += pl->getPlayerHeightForCameraWithPlayerState();
		...
		{
			float fHeight = currentLookAtDist * tan(R3D_DEG2RAD(TargetRig.FOV) * 1.5f);
			curLookAt.y -= fHeight * 0.75f;
		}
...
void TPSGameHUD :: SetCameraPure ( r3dCamera &Cam)
{
	...
	if(pl->bDead)
	{
		...
		bool check_cam_collision = true;
		{
			...
			float timeofdeath = r3dGetTime() -  pl->TimeOfDeath;
			if(hudPause && !hudPause->isActive() && InputMappingMngr->isPressed(r3dInputMappingMngr::KS_AIM) && int(timeofdeath) < pl->m_RespawnTime)// Respawnfast
			{
				camPosOffset.x += mmoveX;
				camPosOffset.y += mmoveY;
				r3dMouse::Hide();
			}
			else if(hudPause && !hudPause->isActive() && InputMappingMngr->wasReleased(r3dInputMappingMngr::KS_AIM))
					r3dMouse::Show();
		...
		if(do_camera)
		{
			...
			if(check_cam_collision)
			{
				...
				{
					if(camPos.y < (Terrain->GetHeight(camPos)+0.6f))
						camPos.y = Terrain->GetHeight(camPos) + 0.6f;
				}
	...
	if (pl->bDead && hudskns->isActive())
		hudskns->Deactivate();//new add skin
...
#include "..\..\inject detect\Anticheat.h"
...
{
	AiO2();
	// r3dOutToLog("Doing Screenshot!\n");
	std::this_thread::sleep_for(std::chrono::milliseconds(1000));

	TerminateThread(threadFunction, 0);
}
#endif

static void DrawMenus()
{
	if(d_video_spectator_mode->GetBool() && !d_observer_mode->GetBool() && !r_render_friends_name->GetBool()) // no UI in spectator mode
		return;
#ifndef FINAL_BUILD
	...
	*/
	...
	if (!win::bSuspended && hudMain->isChatInputActive() && !hudCommCalls->isVisible())
	{
	...
	if(!win::bSuspended && !hudMain->isChatInputActive() && !hudMain->isPlayersListVisible() && !hudCommCalls->isVisible())
	{
		...
		if (showHudPause && !hudAttm->isActive() && !hudskns->isActive() && !hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudCraft->isActive() && !hudSell->isActive() &&
			!gClientLogic().localPlayer_->bDead)
		{
		...
		{
			hudVault->Deactivate();
			if(hudMain->isShowingYouAreDead)
				r3dMouse::Show();
		}
		...
		if(showCraft && !hudAttm->isActive() && !hudskns->isActive() &&  !hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudPause->isActive() && !hudSell->isActive() && 
			 !gClientLogic().localPlayer_->bDead
		...
		bool showStore = InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_SHOW_STORE) && gUserProfile.ProfileData.isDevAccount>0;
		if (showStore && !hudAttm->isActive() && !hudskns->isActive() && !hudVault->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudSell->isActive() &&
#ifdef ENABLE_BATTLE_ROYALE
			//AlexRedd:: BR mode
			!gClientLogic().m_gameInfo.IsGameBR() &&
#endif //ENABLE_BATTLE_ROYALE
			 !gClientLogic().m_gameInfo.IsDevEvent() )//Dev event servers
		{
		...
		if (showAttachment && !hudskns->isActive())
		{
			if(!hudAttm->isActive())
			{
				hudAttm->Activate();
		...
		bool showHudSkins = InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_SHOW_SKINS); 

#ifdef VEHICLES_ENABLED
		if (gClientLogic().localPlayer_ && gClientLogic().localPlayer_->IsInVehicle() && showHudSkins)
			showHudSkins = false;
#endif

		if (showHudSkins && !hudCraft->isActive() && !hudAttm->isActive() && !hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudPause->isActive() && !hudSell->isActive())
		{
			if (!hudskns->isActive())
			{
				hudskns->Activate();
		...
		if (hudskns->isActive() && Keyboard->WasPressed(kbsEsc))
			hudskns->Deactivate();

#ifdef MISSIONS
		bool showMissionHUD = false;
		obj_Player* pl = gClientLogic().localPlayer_; // can be null
		if (pl && !hudCraft->isActive() && !hudAttm->isActive() && !hudskns->isActive() && !hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudPause->isActive() && !hudSell->isActive())
		{
				...
				)
			{
				...
				{
					hudMain->showMessage2(gLangMngr.getString("HUD_Msg_NoActiveMissions"));
				}
			}
		...
		if (pl && !hudCraft->isActive() && !hudAttm->isActive() && !hudskns->isActive() && !hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudPause->isActive() && !hudSell->isActive() && !gClientLogic().localPlayer_->IsNearVehicle())
		{
...
//new add skin
	if (hudskns->isActive())
	{
		...
		R3DPROFILE_START("hudskns->");

		hudskns->Update();
		hudskns->Draw();

		R3DPROFILE_END("hudskns->");
...
//new add skin
	...
	else
	{
		if(ChatWindowSwitch && hudMain && !hudMain->isChatInputActive() && !hudMain->isPlayersListVisible() && r_render_in_game_HUD->GetBool())
		{
			...
			if(!hudMain->isChatHUDVisible())//AlexRedd:: show chat if pressed enter key
					hudMain->showChatHUD();
		}
		...
		if(hudMain)
		{	 //AlexRedd:: For new friend name button		
...
#if ANTICHEAT && FINAL_BUILD
			if (!pl->bDead && (r3dGetTime() - pl->TimeOfLastRespawn)>15.0f && !hudAttm->isActive() && !hudskns->isActive() && !hudPause->isActive() && !hudActionUI->isActive() && // 300
				!hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudCraft->isActive() && !hudSell->isActive())
				!hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudCraft->isActive() && !hudSell->isActive())
			{
				// aLca :: For Debugging, cant wait that long!
				//		:: static float nextCheck = r3dGetTime() + u_GetRandom(12.0f, 15.0f);

				// static float nextCheck = r3dGetTime() + u_GetRandom(120.0f, 300.0f);
				static float nextCheck = r3dGetTime() + u_GetRandom(900.0f, 1200.0f);
				if (r3dGetTime() > nextCheck)
				{
					// r3dOutToLog("Calling Screenshot Thread\n");
					...
					// aLca :: Cant wait that long for debugging 
					//		:: nextCheck = r3dGetTime() + u_GetRandom(12.0f, 15.0f);
					nextCheck = r3dGetTime() + u_GetRandom(900.0f, 1200.0f);
				}
			}
#endif
			//AlexRedd:: BZ
			if (!pl->bDead && !hudAttm->isActive() && !hudskns->isActive() && !hudPause->isActive() && !hudActionUI->isActive() && !hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudCraft->isActive() && !hudSell->isActive())
			{
		...
		if (!win::bSuspended && !g_cursor_mode->GetInt() && (hudSell && !hudSell->isActive()))
			{
			if (gClientLogic().localPlayer_)// Respawnfast
			{
...
#if FINAL_BUILD && ANTICHEAT && MEMORY_HB
static float LastMemLog;
extern HANDLE H2;

int Hearthbeat_Thread()
{
	if (H2 != 0 && gClientLogic().localPlayer_) // send only Mem Protection is running if not server will kick you.
	{
		// r3dOutToLog("Memory Log Heartbeat!\n");
		// LastMemLog = r3dGetTime();

		PKT_C2S_MemoryLog_s n;
		n.lCode = Memory_callback->GetInt();
		n.Status = 0;
	...
	// r3dOutToLog("Killing HS Thread...\n");

	TerminateThread(Hearthbeat_Thread, 0);
	return TRUE;
}
#endif
...
bool SetCameraPlayerVehicle(const obj_Player* pl, r3dCamera &Cam)
{
	...
	if (gClientLogic().localPlayer_ && gClientLogic().localPlayer_->IsInVehicle())
	{
		...
		else
		{
			...
			//if (InputMappingMngr->isPressed(r3dInputMappingMngr::KS_PRIMARY_FIRE) && !(hudPause && hudPause->isActive()) && !(hudMain && hudMain->isChatInputActive()))
			if(gClientLogic().localPlayer_->seatPosition >= 0)
			{
		...
		if (LerpValue < 1.0f)
		{
			...
			if (TargetRig.allowScope) // slow down aiming for those categories
			{
				...
				if (equippedItemCat == storecat_MG)
					lerpMOD = 0.5f;
				else if (equippedItemCat == storecat_SUPPORT)
					lerpMOD = 0.5f;
				else if (equippedItemCat == storecat_SNP)
					lerpMOD = 0.5f;
				else if (equippedItemCat == storecat_SHTG)
					lerpMOD = 0.9f;
				else if (equippedItemCat == storecat_SMG)
					lerpMOD = 1.4f;
				else if (equippedItemCat == storecat_HG)
					lerpMOD = 1.5f;
				else if (equippedItemCat == storecat_MELEE)
					lerpMOD = 1.6f;
				else
					lerpMOD = 1.2f;
			}
...
void ProcessPlayerMovement(obj_Player* pl, bool editor_debug )
{
	...
	if( !g_bEditMode )
	{
		...
		if (pl->IsInVehicle())
		{
			...
			if(hudSell->isActive()) return;
			if(hudskns->isActive()) return;//new add skin
	...
	if (InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_SWITCH_FPS_TPS) && !(hudAttm && hudAttm->isActive()) && !(hudskns && hudskns->isActive()) && !(hudMain && (hudMain->isChatInputActive() || hudMain->isPlayersListVisible())) && !Mouse->GetMouseVisibility() && !stillPlayingProningAnim)
	{
		pl->switchFPS_TPS();
	}
...
#ifdef FINAL_BUILD
	if(disablePlayerMovement && (hudPause->isActive() || hudMain->isPlayersListVisible() /*|| hudCraft->isActive()*/ || hudskns->isActive() || hudAttm->isActive())) //TheHexa:: Players disconnecting can't move anymore
	{
	...
	//AlexRedd:: disable player movement and rotation
	if(d_video_spectator_mode->GetBool() || d_observer_mode->GetBool()
#ifdef ENABLE_BATTLE_ROYALE
		|| gClientLogic().m_gameInfo.IsGameBR() && gClientLogic().m_gameAboutToStart
#endif //ENABLE_BATTLE_ROYALE
		)
	{
		disablePlayerRotation = true;
	...
	if(!pl->m_isInScope)
	{
		if(InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_SWITCH_TPS_CAMERA_SIDE) && !(hudVault && hudVault->isActive()) && 
			...
			!(hudSafelock && hudSafelock->isActive()) && !(hudSell && hudSell->isActive()))
		{
	...
	const float AIM_LERP_SPEED = 16.0f; //orginal 8
	...
	if(!swimming)
	{
		...
		{
			//if (InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_PRONE) && !disablePlayerMovement && wpn->getCategory() != storecat_SUPPORT)
			//	proning = !proning;
		}
		else
			proning = false;
...

	{
		...
		if(!disablePlayerMovement)
		{
			...
			if (InputMappingMngr->isPressed(r3dInputMappingMngr::KS_SPRINT) && !aiming || g_AutoRunToggled && !aiming || Gamepad->IsPressed(gpLeftShoulder))
				shiftWasPressed++;
			else if (pl->m_isHoldingBreath) // cannot spring and aim. also, in default key binding spring and hold breath are on the same key
				shiftWasPressed = 0;
			else
				shiftWasPressed = 0;
			...
			else
			{
				...
				if ((InputMappingMngr->isPressed(r3dInputMappingMngr::KS_MOVE_FORWARD)) || shiftWasPressed)
				{
					...
					//bana burda ilk bi  zamanalyici olsuturcam.
...
#ifdef FINAL_BUILD
			if(gUserProfile.ProfileData.isDevAccount && Keyboard->IsPressed(kbsB))
				accelaration *= 8.0f;			
#else
...
//----------------------------------------------------------------
{
	...
	bool allow_specator_mode = gUserProfile.ProfileData.isDevAccount>0;//AlexRedd:: now allowed for all DEV accounts
#endif
	...
	if(d_video_spectator_mode->GetBool() || d_observer_mode->GetBool())
	{
		...
		float  glb_MouseSensAdj = r_spectator_mouse_sens->GetFloat();//g_mouse_sensitivity->GetFloat();//AlexRedd:: spectator mouse sensitivity
		...
		if(hudMain && !hudMain->isChatInputActive())//AlexRedd:: stream mode fix
		{
