...
void CharacterHUD :: Process()
{
	...
	float  glb_MouseSensAdj = r_spectator_mouse_sens->GetFloat();//1.0f;	// in range (0.1 - 1.0)
	...
	if (Keyboard->IsPressed(kbsQ)) FPS_Position.Y += SRV_WORLD_SCALE(1.0f) * fSpeed * r3dGetFrameTime() * mult;
	if (Keyboard->IsPressed(kbsE)) FPS_Position.Y -= SRV_WORLD_SCALE(1.0f)* fSpeed * r3dGetFrameTime() * mult;
