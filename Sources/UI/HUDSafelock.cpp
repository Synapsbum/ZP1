...
void HUDSafelock::Update()
{
	if(isAirdrop)
	{	
		float timeLeft = m_airdropTimeLeft - r3dGetTime();
		...
		char airdropExpireTimeText[128] = { 0 };
		sprintf(airdropExpireTimeText, "%s %s", gLangMngr.getString("$FR_AIRDROP_WILL_DESTROY_IN"), tempStr);
		gfxMovie.SetVariable("_root.api.Main.ExpireTime.text", airdropExpireTimeText);
	}
}
...
void HUDSafelock::Activate()
{
	...
	if(isVehicle)
	{
		gfxMovie.SetVariable("_root.api.Main.BtnPickup.visible", false);
		gfxMovie.SetVariable("_root.api.Main.ExpireTime.visible", false);
		gfxMovie.SetVariable("_root.api.Main.Title1.text", gLangMngr.getString("$FR_VEHICLELOCK_TITLE"));		
	}
	else if(isAirdrop)
	{
		gfxMovie.SetVariable("_root.api.Main.BtnPickup.visible", false);
		gfxMovie.SetVariable("_root.api.Main.ExpireTime.visible", true);
		gfxMovie.SetVariable("_root.api.Main.Title1.text", gLangMngr.getString("$FR_AIRDROP_TITLE"));
	}
	else{
		gfxMovie.SetVariable("_root.api.Main.BtnPickup.visible", true);
		gfxMovie.SetVariable("_root.api.Main.ExpireTime.visible", false);
		gfxMovie.SetVariable("_root.api.Main.Title1.text", gLangMngr.getString("$FR_SAFELOCK_TITLE"));
	}
