...
//////////////////////////////////////////////////////////////////////////
	...
	void DrawInstancedMesh(r3dMesh * pMesh, int iCount, float dsp, int i)
	{
		...
		{
			{
				...
				pMesh->DrawMeshInstances(iCount, i);
			}
...
#ifndef FINAL_BUILD
	r3dOutToLog( "CollectionsManager::Init: populating %d objects... \n", elements.Count() );
#endif
...
bool CollectionsManager::LoadTypesFromXML()
{
	...
	while (collTypeDesc)
	{
		...
		GetXMLVal("lod3_dist", collTypeDesc, &newType->lod3Dist);
		int tmpI = 0;
...
void CollectionsManager::SaveTypesToXML()
{
	...
	for (uint32_t i = 0; i < types.Count(); ++i)
	{
		...
		SetXMLVal("lod3_dist", collTypeDesc, &newType->lod3Dist);
...
bool CollectionsManager::LoadElementsFile(const char * fname, float loadProgressStart, float loadProgressEnd)
{
	...
	if (f->size < sizeof(COLL_ELEMS_HEADER_V1))
	{
		fclose(f);
#ifndef FINAL_BUILD
		r3dOutToLog("COLLECTIONS: Collection elements file is corrupted: '%s'\n", fname);
#endif
		return false;
	}
	...
	if (hdr != COLL_ELEMS_HEADER_V1)
	{
		fclose(f);
#ifndef FINAL_BUILD
		r3dOutToLog("COLLECTIONS: Unsupported collection elements file format: '%s'\n", fname);
#endif
		return false;
	}
	...
	fclose(f);
#ifndef FINAL_BUILD
	r3dOutToLog( "Loading collection elements from file: %s...\n", fname );
#endif
...
void CollectionsManager::ComputeVisibility(bool shadows, bool directionalSM)
{
	...
	for (uint32_t i = 0; i < arrSize;)
	{
		...
		const float loddist3 = t.lod3Dist;
		...
		e.curLod = 0;
		if (distSq > loddist3 * loddist3 && t.meshLOD[3])
		{
			e.curLod = 3;
		}
		else if (distSq > loddist2 * loddist2 && t.meshLOD[2])
		{
			e.curLod = 2;
		}
	...
	while (i < numObjects)
	{
		...
		while (i < numObjects && elements[visibleObjects[i]].curLod == ce.curLod && elements[visibleObjects[i]].typeIndex == ce.typeIndex)
		{
			...
			if (pMesh)
			{
				...
				{
					for (int j = 0; j < pMesh->NumMatChunks; j++)
					{
						...
						if (bShadowMap)
							pMesh->MatChunks[j].Mat->StartShadows(vsType);
						else
							pMesh->MatChunks[j].Mat->Start(vsType, drawMode == R3D_IDME_DEPTH ? R3D_MATF_NO_PIXEL_SHADER : 0);

						DrawInstancedMesh(pMesh, iCount, highlighOffset, j);

						if (bShadowMap)
							pMesh->MatChunks[j].Mat->EndShadows();
						else
							pMesh->MatChunks[j].Mat->End();
					}
