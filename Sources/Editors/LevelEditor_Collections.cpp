...
void Draw_Collection_Toolbar()
{
	...
	{
		static const r3dString masksPath = "Data\\TerrainData\\CollectionMasks\\";
		static float fileOffset = 0;
		imgui_FileList(SliderX - 370, 100.0f, DEFAULT_CONTROLS_WIDTH, 300, (masksPath + "*.dds").c_str(), maskName, &fileOffset, false);
	...
	case CE_EDITLAYERS:
		{
			for (uint32_t i = 0; i < gCollectionsManager.GetTypesCount(); i++)
			{
				...
				if(currentGroup.ExpandedProperties)
				{
					...
					SliderY+=imgui_Value_Slider(SliderX+offset, SliderY, "LOD3 Dist",		&t->lod3Dist, 0.0,9999,"%-02.0f",1);
