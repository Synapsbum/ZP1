...
#include "../ObjectsCode/Gameplay/BattleZone.h"//AlexRedd:: BZ
...
int LoadLevel(float startLoadingProgress)
{
	...
	///////////////////////////////////////////////////////////////////////////////////// OLD
	{
		...
		else if (g_disable_grass->GetBool() && !g_bEditMode)
		{
			LoadGrassLib();
			g_pGrassMap->Load(r3dGameLevel::GetHomeDir());
#ifndef FINAL_BUILD
...
bool g_bStartedAsPhysXEditor = false;
bool g_bStartedAsCharacterEditor = false;
...
// FUCK OFF WHO EVER READDED THIS NON-SENSE HARD LINK BULLSHIT!
// this file will be included in shaders and in c++ code!

//#define USE_PCF 1
#define USE_VSM 1 // testing...
...
namespace
{
	...
	void UpdateBattleZonePlaneGrids(float fCellSize)//AlexRedd:: BZ
	{
		if (Terrain3 && Terrain3->IsLoaded())
		{
...
void RenderLevelMinimap(const char* TargetFile)
{
	...
	int texWidth = 4096;// from 2048
	int texHeight = 4096;// from 2048
...
void Editor_Level::SaveLevel(const char* targetDir, bool showSign, bool autoSave)
{
	...
	//if(strcmp(targetDir,"Levels\\wz_brmap")==0 || strcmp(targetDir,"Levels\\alpha_island")==0)//AlexRedd:: BR mode
	g_BattleZone.SaveBattleZoneGrid(targetDir);//AlexRedd:: BZ
...
void Editor_Level::Process(bool enable)
{
	...
	const static char* list[12] = { "Exit Level Editor", "Settings", "Terrain", "Objects", "Materials", "Environment", "Collections", "Decorators", "Roads", "Gameplay", "Post FX", "Color Correction" };
	const int old_MainToolIdx = MainToolIdx;
	imgui_Toolbar(5, 5, r3dRenderer->ScreenW - 10, 35, &MainToolIdx, Editor_Level::EDITMODE_LEAVE, list, sizeof list / sizeof list[0], false);
	...
	//AlexRedd:: reload objects
	if (imgui_Button(5, r3dRenderer->ScreenH - 235, DEFAULT_CONTROLS_WIDTH, DEFAULT_CONTROLS_HEIGHT, "Reload Object Categories"))
	{
		ReloadObjects();
	}
	...
	//AlexRedd:: hide profiler
	/*int hideProfiler = 0;
	hideProfiler = r_profiler_d3d->GetInt();
	imgui_Checkbox(370, r3dRenderer->ScreenH - 35 - 25 - 35 - 35 - 35 - 35 - 35 - 35, "Profiler d3d", &hideProfiler, 1);
	r_profiler_d3d->SetInt(hideProfiler ? 1 : 0);*/

	//AlexRedd:: hide player debug data
	int hideDebugData = 0;
	hideDebugData = r_show_player_debug_data->GetInt();
	imgui_Checkbox(370, r3dRenderer->ScreenH - 35 - 25 - 35 - 35 - 35 - 35 - 35 - 35, "Show player debug data", &hideDebugData, 1);
	r_show_player_debug_data->SetInt(hideDebugData ? 1 : 0);

	//AlexRedd:: hide icons
	int hideIcons = 0;
	hideIcons = r_hide_icons->GetInt();
	imgui_Checkbox(370, r3dRenderer->ScreenH - 35 - 25 - 35 - 35 - 35 - 35 - 35, "Hide Icons", &hideIcons, 1);
	r_hide_icons->SetInt(hideIcons ? 1 : 0);

	//AlexRedd:: hide status bar
	int hideEditorStatusBar = 0;
	hideEditorStatusBar = r_hide_editor_statusbar->GetInt();
	imgui_Checkbox(370, r3dRenderer->ScreenH - 35 - 25 - 35 - 35 - 35 - 35, "Hide Status Bar", &hideEditorStatusBar, 1);
	r_hide_editor_statusbar->SetInt(hideEditorStatusBar ? 1 : 0);

	//AlexRedd:: spectator mouse sensitivity
	int use_lerp_cam = r_editor_use_lerp->GetBool();
	imgui_Checkbox(370, r3dRenderer->ScreenH - 35 - 25 - 35 - 35 - 35, "Lerp Camera", &use_lerp_cam, 1);
	r_editor_use_lerp->SetBool(use_lerp_cam);

	float editor_sens = r_spectator_mouse_sens->GetFloat();
	imgui_Value_Slider(370, r3dRenderer->ScreenH - 35 - 25 - 35 - 35, "Spectator sens", &editor_sens, 0.0f, 1.0f, "%.2f", 1, false);
	r_spectator_mouse_sens->SetFloat(editor_sens);

	float editor_lerp_speed = r_editor_lerp_speed->GetFloat();
	imgui_Value_Slider(370, r3dRenderer->ScreenH - 35 - 25 - 35, "Reduce smoothness", &editor_lerp_speed, 1.0f, 10.0f, "%.0f", 1, false);
	r_editor_lerp_speed->SetFloat(editor_lerp_speed);

	float editor_speed = r_editor_move_speed->GetFloat();
	imgui_Value_Slider(370, r3dRenderer->ScreenH - 35 - 25, "Speed", &editor_speed, 0.1f, 50.0f, "%.2f", 1, false);
...

	{
		...
		if (pPickedObj && pPickedObj->GetObjStat(sStatStr, MAX_PATH - 1))
		{
			...
			imgui_Static(738, r3dRenderer->ScreenH - 35 - 25 - 92, sStatStr, DEFAULT_CONTROLS_WIDTH, true, fStatisticHeight + 20);
		}
	}
	...
	if (MainToolIdx == EDITMODE_LEAVE)
	{
		MainToolIdx = old_MainToolIdx;

		auto msg = MessageBox(r3dRenderer->HLibWin, "Do you want to save your currently map progress?", "Confirm", MB_YESNOCANCEL);
		if (msg == IDYES)
		{
			SaveLevel(r3dGameLevel::GetHomeDir(), true, false);
			leave = true;
					  ...
					  case SS_MISC:
					  {
									  ...
									  // z prepass settings
						  {
									  ...
									  SliderY += imgui_Static(SliderX, SliderY, "Mouse Smoothing");
									  {
										  ...
										  SliderY += imgui_Value_SliderI(SliderX + 5, SliderY, "Num Frames", &g_mouseSmoothingNumFrames, 0, 60, "%d");
...
float Editor_Level::ProcessTerrain3_HMap(float SliderX, float SliderY)
{
	...
	if (imgui_Button(SliderX, SliderY, 360, 25, "Export heightmap", 0))
	{
		...
		_splitpath(r3dGameLevel::GetHomeDir(), drive, folder, name, ext);
...
void FillCatFromDirRecursive(CategoryStruct& cat, const char* clazz, const r3dSTLString &path, const char *ext, const r3dSTLString &relFolder)
{
	...
	if (h != INVALID_HANDLE_VALUE)
	{
		do
		{
			...
			{
				//AlexRedd:: filters
				if (g_hide_playeronly_models->GetBool())
				{
					if (stristr(ffblk.cFileName, "_playerOnly.sco"))
						continue;
				}
				if (g_hide_collision_models->GetBool())
				{
					if (stristr(ffblk.cFileName, "_collision.sco") || stristr(ffblk.cFileName, "_Collision.sco"))
						continue;
				}
				if (g_hide_lods_models->GetBool())
				{
					if (stristr(ffblk.cFileName, "_lod1.sco") || stristr(ffblk.cFileName, "_lod2.sco") || stristr(ffblk.cFileName, "_lod3.sco"))
						continue;
				}
...
void Editor_Level::ProcessObjects()
{
	...
	int _objXRotate = Keyboard->IsPressed(kbsLeftAlt);
	int _objYRotate = Keyboard->IsPressed(kbsLeftShift);
	int _objZRotate = Keyboard->IsPressed(kbsCapsLock);
	...
	//AlexRedd:: filters
	int hidePlayeronlyModels = 0;
	hidePlayeronlyModels = g_hide_playeronly_models->GetInt();
	SliderY += imgui_Checkbox(SliderX, SliderY, "Hide PlayerOnly Models", &hidePlayeronlyModels, 1);
	g_hide_playeronly_models->SetInt(hidePlayeronlyModels ? 1 : 0);

	int hideCollisionModels = 0;
	hideCollisionModels = g_hide_collision_models->GetInt();
	SliderY += imgui_Checkbox(SliderX, SliderY, "Hide Collision Models", &hideCollisionModels, 1);
	g_hide_collision_models->SetInt(hideCollisionModels ? 1 : 0);

	int hideLodModels = 0;
	hideLodModels = g_hide_lods_models->GetInt();
	SliderY += imgui_Checkbox(SliderX, SliderY, "Hide LOD Models", &hideLodModels, 1);
	g_hide_lods_models->SetInt(hideLodModels ? 1 : 0);
	...
	case 0:
	{
			  ...
			  if (iEditMode)
			  {
				  ...
				  if (EditObject)
				  {
					  ...
					  if (!_objYRotate && !_objXRotate && !_objZRotate)
					  {
			  ...
			  if (iEditMode)
			  {
				  if (_objXRotate) // X rotate
				  {
					  XRotation += imgui_mmx / 2.0f;
					  if (XRotation > 360.0f) XRotation -= 360.0f;
					  if (XRotation <  0.0f) XRotation = 360.0f - XRotation;

					  EditObject->SetRotationVector(r3dVector(XRotation, 0, 0));
				  }
				  if (_objYRotate) // Y rotate
				  {
				  ...
				  if (_objZRotate) // Z rotate
				  {
					  ZRotation += imgui_mmx / 2.0f;
					  if (ZRotation >  360.0f) ZRotation -= 360.0f;
					  if (ZRotation <  0.0f) ZRotation = 360.0f - ZRotation;

					  EditObject->SetRotationVector(r3dVector(0, 0, ZRotation));
				  }
			...
			if (imgui_Button(SliderX + 125, SliderY, 115, 25, "Export NoClip"))
			{
				...
				if (obj_NoClipBox::s_vNoClipData.size() > 0 || obj_NoClipPlane::noClipDatas.size() > 0)
				{
					...
					if (mapname)
					{
						...
						if (stricmp(mapname + 1, "ZP_Test") == 0)
							mapId = GBGameInfo::MAPID_ZP_Test;
						...
						else if (stricmp(mapname + 1, "wz_aircraftcarrier") == 0)
							mapId = GBGameInfo::MAPID_WZ_AircraftCarrier;
						else if (stricmp(mapname + 1, "wz_trade_map") == 0)
							mapId = GBGameInfo::MAPID_WZ_Trade_Map;
						else if (stricmp(mapname + 1, "zh_warehouse") == 0)
							mapId = GBGameInfo::MAPID_ZH_Warehouse;
						else if (stricmp(mapname + 1, "wz_brmap") == 0)
							mapId = GBGameInfo::MAPID_WZ_BRmap;
						else if (stricmp(mapname + 1, "wz_clearview_v2") == 0)
							mapId = GBGameInfo::MAPID_WZ_Clearview_V2;
						else if (stricmp(mapname + 1, "wz_nevada") == 0)
							mapId = GBGameInfo::MAPID_WZ_Nevada;
						else if (stricmp(mapname + 1, "wz_rockyford") == 0)
							mapId = GBGameInfo::MAPID_WZ_RockyFord;
						else if (stricmp(mapname + 1, "wz_quarantine") == 0)
							mapId = GBGameInfo::MAPID_WZ_Quarantine;
					}
					else
					{
...
void Editor_Level::ProcessMeshes()
{
	...
	if (nIndex >= 0 && nIndex < int(objectsNames.size()))
	{
		...
		imgui_Static(738, r3dRenderer->ScreenH - 200, buffer, 360, true, 120);
	}
}
...
void ProcessBattleZoneEditor()//AlexRedd:: BZ
{
	float SliderX = r3dRenderer->ScreenW - 375;
	float SliderY = 85;
	...
	if (bJustStarted)
		UpdateBattleZonePlaneGrids(g_fBattleZoneCellSize);
...
void Editor_Level::ProcessMaterials()
{
	...
	if (obj)
	{
			...
			)
		{
			if (UI_TargetMaterial)
			{
				Font_Label->PrintF(20, 105, r3dColor(255, 255, 255), "MAT: %s [%s]", UI_TargetMaterial->Name, UI_TargetMaterial->DepotName);
			}
	...
	if (EditMaterial == 0) // show material list
	{
		...
		imgui_Static(5, 130, "Material List:", 300);
		if (imgui_DrawList(5, 155, 300, 450, matNames, &arrOffset, &curSelection, true, false, true))
			EditMaterial = ppMatLibrary[curSelection];
	}

	//	int selectMatID = -1
	static int showExtendedMaterialsEditor = 0;
	static int showMaterialTypeEdit = 0;
	if (EditMaterial)
	{
		...
		imgui_Static(SliderX, SliderY, "Material Type:", 200);
		if (imgui_Button(SliderX + 200, SliderY, 160, 25, "EDIT", showMaterialTypeEdit))
		{
			showMaterialTypeEdit = !showMaterialTypeEdit;
		}
		SliderY += 26;
		...
		if (imgui_Button(SliderX + 10 + 2 + 106, SliderY - 10, 180, 20, "DELETE", 0))
		{
			if (MessageBoxA(r3dRenderer->HLibWin, "This will delete this texture file and save level materials. Are you sure?", "WARNING", MB_YESNO) == IDYES)
			{
				if (MatTexArray[i])
				{
		...
		if (imgui_Button(SliderX + 10 + 2 + 106, SliderY + 32 + 2, 180, 20, "EDIT", 0))
		{
			if (MatTexArray[i])
			{
#ifndef FINAL_BUILD
		...
		if (MatTexArray[i])
		{
			int nMips = MatTexArray[i]->GetNumMipmaps() < 0 ? 0 : MatTexArray[i]->GetNumMipmaps();
			char szFormat[24];
			char buffer[256];
			ToString(MatTexArray[i]->GetD3DFormat(), szFormat);
			sprintf_s(buffer, 256, "Format: %s Size: %ix%i Mips: %i", szFormat, MatTexArray[i]->GetWidth(), MatTexArray[i]->GetHeight(), nMips);
			imgui_Static(SliderX + 22 + 100, SliderY + 60, buffer, 235);
		}
		...
		imgui_Static(5, 130, "Texture List:", 300);

		static float offset = 0.f;

		if (imgui_FileList(5, 155, 300, 450, DirName, MatFName, &offset, false, bChangedVal))
		{
			char SS1[256];
	...
	if (showMaterialTypeEdit)
	{
		g_pDesktopManager->Begin("ed_material_type");
...
void Editor_Level::Init()
{
	...
	pDesc = g_pDesktopManager->AddDesktop("ed_material_type");
	pDesc->SetViewPosition(r3dRenderer->ScreenW - 380 - 362, 50);
	pDesc->SetViewSize(362, r3dRenderer->ScreenH - 50);
...
void DebugPlayerDie()
{
	if (g_bEditMode)
	{
		for (GameObject *obj = GameWorld().GetFirstObject(); obj; obj = GameWorld().GetNextObject(obj))
		{
			if (obj->isObjType(OBJTYPE_Human))
			{
				static_cast<obj_Player*>(obj)->DoDeath(0, true, storecat_INVALID, 0, false);
			}
		}
	}
}
...
void DisplayLumaInfo()
{
	...
	imgui_Static(5, r3dRenderer->ScreenH - 270, exp_val);
}
